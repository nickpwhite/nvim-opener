...
if [[ -f "$THREAD_SYNC_PLIST" ]]; then
  LAUNCH_DOMAIN="gui/$(id -u)"
  launchctl bootout "$LAUNCH_DOMAIN/$THREAD_SYNC_LABEL" >/dev/null 2>&1 || true
  launchctl disable "$LAUNCH_DOMAIN/$THREAD_SYNC_LABEL" >/dev/null 2>&1 || true
  rm -f "$THREAD_SYNC_PLIST"
fi
...
echo "Removed thread sync launch agent: $THREAD_SYNC_PLIST"
