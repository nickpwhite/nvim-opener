...
- Reconciles tmux windows against active Codex threads and cleans up orphaned worktrees.
...
- `--sync-thread-state`:
  - Reads non-archived Codex threads from `~/.codex/state_5.sqlite`.
  - Compares active thread `cwd` values to managed tmux worktrees.
  - Closes managed tmux windows whose worktree no longer has any active Codex thread.
  - Removes the matching worktree nvim socket when cleanup runs.

The macOS installer sets up a LaunchAgent (`com.nick.nvim-opener.thread-sync`) that runs `--sync-thread-state` every 15 seconds (`RunAtLoad=true`).

LaunchAgent logs:

- `~/Library/Logs/nvim-opener-thread-sync.log`
- `~/Library/Logs/nvim-opener-thread-sync.err.log`
